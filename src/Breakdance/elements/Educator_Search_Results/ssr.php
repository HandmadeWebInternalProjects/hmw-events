...
if (!empty($_GET['course_type'])) {
  $raw_course_types = explode(',', sanitize_text_field(wp_unslash($_GET['course_type'])));
  $filters['course_type'] = array_values(array_filter(array_map('sanitize_text_field', $raw_course_types)));
}
...
if ($location) {
  ...
  $result = search_educators_by_state($state, $paged, $per_page, $filters);
  $educators = $result['educators'];
  $total = $result['total'];
  $max_pages = $result['max_pages'];
} elseif ($country) {
...
<div class="educator-search-results">

  <?php if (!empty($filters['course_type'])) : ?>
    <div class="active-filters">
      <span class="active-filters__label">Filtered by:</span>
      <?php foreach ($filters['course_type'] as $slug) :
        $term  = get_term_by('slug', $slug, 'course_type');
        $label = $term ? $term->name : $slug;
        $remaining = array_values(array_diff($filters['course_type'], [$slug]));
        $base_url  = remove_query_arg('pg');
        $remove_url = empty($remaining)
          ? remove_query_arg('course_type', $base_url)
          : add_query_arg('course_type', implode(',', $remaining), $base_url);
      ?>
  ...
  <?php if (empty($educators)) : ?>
    <div class="no-results">
      <p>No educators found matching your search criteria.</p>
    </div>
  <?php else : ?>
