 ...
 * @return int[]|null
 */
function get_educator_ids_for_filters(array $filters): ?array {
  ...
  $posts = get_posts([
    ...
    'tax_query'      => [[
      ...
      'field'    => 'slug',
      'terms'    => (array) $filters['course_type'],
    ]],
  ]);
...
/**
 * Search educators by location (postcode or suburb) — searches educator profiles directly.
 *
 ...
 * @param array $filters Optional filters (e.g. ['course_type' => ['slug1', 'slug2']])
...
function search_educators_by_location_direct($location, $paged = 1, $per_page = 10, $filters = [], $radius_km = 2000, $country = '') {
  ...
  $filtered_educator_ids = get_educator_ids_for_filters($filters);
...
/**
 * Search educators by location (postcode or suburb) — searches via educator courses.
 *
...
function search_educators_by_location($location, $paged = 1, $per_page = 10, $filters = [], $country = '') {
  ...
  if ($country) {
    $educators = array_filter($educators, function ($educator) use ($country) {
      $edu_country = get_field('educator_country_code', 'user_' . $educator->ID);
 ...
 */
function search_educators_by_state($state, $paged = 1, $per_page = 10, $filters = []) {
  $filtered_educator_ids = get_educator_ids_for_filters($filters);
  if ($filtered_educator_ids !== null && empty($filtered_educator_ids)) {
...
/**
 * Search educators by country (for non-Australian educators)
 *
...
function search_educators_by_country($country, $paged = 1, $per_page = 10, $filters = []) {
  ...
  $user_args = [
    ...
    'meta_query' => [
      'relation' => 'OR',
      [
        'key'     => 'educator_country_code',
        'value'   => $country,
        'compare' => '=',
      ],
      [
        'key'     => 'educator_country',
        'value'   => $country,
        'compare' => '=',
      ],
    ],
 ...
 */
function get_educator_upcoming_courses($educator_id, $limit = 5)
{
  $args = [
    'post_type' => 'educator_course',
    'author' => $educator_id,
 ...
 */
function get_educator_total_upcoming_courses($educator_id)
{
  $args = [
    'post_type' => 'educator_course',
    'author' => $educator_id,
