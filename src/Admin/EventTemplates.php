...
?>
    <div class="wrap">
      ...
      <table class="widefat striped" style="max-width: 980px; margin-bottom: 24px;">
        ...
        <tbody>
          <?php if (empty($templates)) : ?>
            <tr>
              <td colspan="6"><?php esc_html_e('No templates found yet.', 'hmw-events'); ?></td>
            </tr>
          <?php else : ?>
      ...
      <form id="hmwevents-template-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="max-width: 1080px;">
        ...
        <!-- Attendance Options -->
        <div class="hmwevents-section">
          <h3><?php esc_html_e('Attendance Options', 'hmw-events'); ?></h3>
          <p class="description"><?php esc_html_e('Booking options shown on the registration form. Drag to reorder. Each option has a type, a label, and a pricing style (flat price, per attendee, or age bands). The option type determines how many event places one booking uses — Individual books one place, Couple books two, Parent + Child books one adult plus children. "Maximum bookings" caps the number of bookings for this option (leave empty for unlimited).', 'hmw-events'); ?></p>
          <div id="hmwevents-attendance-options-container"></div>
        </div>
        ...
        <div id="hmwevents-advanced-edit">
          ...
          <p class="description"><?php esc_html_e('Edit the raw JSON template data directly. Changes are saved when you click "Create Template" or "Update Template" at the bottom.', 'hmw-events'); ?></p>
        </div>
  ...
  private function get_registry_presets_for_editor(): array
  {
    ...
    foreach ($all as $slug => $config) {
      $presets[$slug] = [
        ...
        'attendance_option_presets' => $config['attendance_option_presets'] ?? [],
      ];
    }
