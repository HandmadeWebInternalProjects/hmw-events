 ...
 * Educator New Booking Notification Handler.
 */
class EducatorNewBookingHandler extends AbstractEmailHandler
{
    /**
     * Template key.
     *
    ...
    public function queue_for_booking($booking_id, $booking_data = [])
    {
        ...
        if (!empty($booking_data['educator_id'])) {
            ...
            $row = $wpdb->get_row($wpdb->prepare(
                "SELECT b.course_post_id FROM {$wpdb->prefix}educator_bookings b WHERE b.id = %d",
                $booking_id
            ));
            if ($row) {
    ...
    protected function prepare_fresh_template_data($booking_id)
    {
        ...
        $booking = $wpdb->get_row($wpdb->prepare(
            "SELECT b.* FROM {$wpdb->prefix}educator_bookings b WHERE b.id = %d",
            $booking_id
        ));
        ...
        $customer_email   = $customer_post_id
            ? (get_post_meta($customer_post_id, 'customer_email', true) ?: '')
            : '';
